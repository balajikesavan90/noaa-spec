...
NOAA-Spec is open-source software for NOAA Integrated Surface Database (ISD) / Global Hourly preprocessing that packages NOAA-specific field interpretation, sentinel handling, quality-code treatment, and canonical cleaned-output generation into a reusable software package. The package implements parsing and cleaning for fixed-width and comma-encoded fields while preserving explicit quality semantics and provenance. This revision demonstrates the canonical cleaning path on tracked reproducibility fixtures with checksum-backed expected-output verification. The software is designed for transparent preprocessing rather than downstream analysis: it reduces silent divergence between project-specific ISD cleaning scripts by making preprocessing decisions inspectable, stable, and repeatable on a reviewer-visible example. By publishing these NOAA-specific preprocessing semantics as versioned software contracts, NOAA-Spec makes cleaned ISD inputs easier to audit, compare, and reuse across studies.

# Summary

NOAA-Spec is a reusable NOAA-specific preprocessing package for NOAA ISD / Global Hourly records. It translates encoded observations into stable cleaned outputs with explicit handling of field semantics, sentinel values, quality codes, provenance, and bounded reproducibility. NOAA ISD is a widely used observational resource for weather and climate research, but its raw records are not analysis-ready [@smith2011isd; @noaa_isd_docs]. Users must interpret fixed-width fields, repeated section families, sentinel values, and quality codes from technical documentation before they can produce comparable cleaned datasets.

NOAA-Spec provides a software-first alternative to keeping those NOAA-specific decisions in local scripts. It packages codified NOAA field semantics, deterministic canonical cleaning, explicit output contracts, and provenance-aware validation into one inspectable software surface that can be reused across studies. The practical contribution is a stable preprocessing layer that reduces silent divergence in how different analysts turn the same raw ISD records into cleaned inputs. In this revision, the reviewer-visible contribution is intentionally narrow: the repository demonstrates the canonical cleaning path and deterministic sample-output verification on tracked fixtures, not a fully reviewer-reproduced publication system.
...
The consequence is visible in a concrete repository example. For station `16754399999` (KARPATHOS, GR), the committed quality report records sentinel replacements for `temperature_c`, and the committed aggregation report records separate canonical temperature-quality/status columns such as `temperature_quality_code`, `TMP__qc_pass`, `TMP__qc_reason`, and `TMP__qc_status`. That is exactly the class of preprocessing choice that often remains implicit in local scripts: one workflow may coerce sentinel-coded temperature values into numeric intermediates or collapse the associated status semantics, while another nulls them and preserves the quality context separately. NOAA-Spec prevents that divergence by making sentinel handling and canonical temperature/status columns explicit in one reusable cleaner.
...
For researchers, the important capability is straightforward: NOAA-Spec provides a reusable preprocessing surface that turns the same raw ISD input and configuration into the same cleaned output with explicitly defined handling of sentinels, field semantics, and quality codes. By ensuring that equivalent inputs and configurations yield identical cleaned outputs with explicitly defined semantics, NOAA-Spec supports consistent reuse, comparison, and audit of ISD-based analyses.

As published software, NOAA-Spec is a versioned preprocessing layer between NOAA raw records and downstream analysis. The reusable unit in this submission is the canonical cleaner exposed through the package and CLI, together with its explicit output contracts and bounded reproducibility checks. The package combines NOAA-specific parsing and cleaning rules derived from documentation and explicit engineering safeguards with provenance-aware validation, while preserving quality and provenance signals during cleaning.
