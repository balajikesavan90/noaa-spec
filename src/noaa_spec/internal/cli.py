"""Internal command-line interface for maintainer workflows."""
...
def _load_pulled_station_state(raw_pull_state_csv: Path) -> pd.DataFrame:
    if not raw_pull_state_csv.exists():
        raise FileNotFoundError(
            ...
            "`poetry run python -m noaa_spec.internal.cli materialize-raw-pull-state` first."
        )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Internal NOAA-Spec maintainer CLI for batch runs, manifests, reports, "
            "and other non-public workflows."
        )
    )
