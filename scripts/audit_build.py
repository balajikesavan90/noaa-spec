...
from noaa_climate_data.cleaning_runner import _checksum_for_output_bundle, _publication_checksum_check
...
def _paths(build_root: Path) -> AuditPaths:
...
def main() -> None:
    ...
    canonical_station_dirs = sorted(path.name for path in audit_paths.canonical_root.iterdir() if path.is_dir())
    domain_station_dirs = sorted(path.name for path in audit_paths.domains_root.iterdir() if path.is_dir())
    station_quality_profiles = sorted((audit_paths.quality_root / "station_quality").glob("station_*.json"))
